...
async def llm_model_func_google(
    prompt, system_prompt=None, history_messages=[], keyword_extraction=False,image=None, **kwargs
) -> str:
    ...
    if system_prompt:
        return response.text + f" <REFERENCES>{",".join(images)}"
    # Return the response text
    return response.text


async def llm_model_func(prompt, system_prompt=None, history_messages=[], keyword_extraction=False,image=None,**kwargs) -> str:
    combined_prompt = ""
...
def main():
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            response,images = response.split("<REFERENCES>")
